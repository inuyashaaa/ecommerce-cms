...
class ApiController
{
    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        if (count($routeValues) > 0) {
            ...
            $action = $routeValues[0];
            $object = @$routeValues[1];
            if (EmptyValue($object) && Post(Config("API_OBJECT_NAME")) !== null) { // Get from Post
                $object = Post(Config("API_OBJECT_NAME"));
            }
            $pageName = "";
            $table = "";
            $field = "";
