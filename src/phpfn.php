...
function Route($i = null)
{
    ...
    if (IsApi() && $i == "key") { // Get record key separated by key separator (for API "/file/object/field/key" action)
        $routeValues = array_slice($routeValues, 3);
        return implode(Config("COMPOSITE_KEY_SEPARATOR"), $routeValues);
    } elseif (is_int($i) || is_string($i)) { // Get route value by index or name
